 ...
 * 屏幕空间光栅化函数 - 将三角形转换为像素
 * 
 * @param t 要光栅化的三角形（包含顶点、颜色、法向量、纹理坐标等信息）
 * @param view_pos 三角形三个顶点在视空间中的位置（用于透视校正插值）
 * 
 * 主要步骤：
...
{
    // 获取三角形的三个顶点坐标（已经过MVP变换和视口变换）
    auto v = t.toVector4();

    // === 步骤1：计算包围盒边界 ===
    float max_x, min_x, max_y, min_y;

    // 计算X轴边界：找到三个顶点中X坐标的最大值和最小值
    ...
    for (int i = min_x; i < max_x; i++) {
        for (int j = min_y; j < max_y; j++) {
            ...
            // === 步骤3：内部测试 - 判断当前像素是否在三角形内部 ===
            if (insideTriangle(i, j, t.v) == true) {
                
                // === 步骤4：计算重心坐标和透视校正插值 ===
                // 计算像素(i,j)相对于三角形的重心坐标
                ...
                // === 步骤5：深度测试（Z-Buffer算法）===
                float index = get_index(i, j); // 获取像素在深度缓冲区中的索引
                
                // 如果当前像素的深度小于等于深度缓冲区中的值，则通过深度测试
                if (zp <= this->depth_buf[index]) {
                    
                    // === 步骤6：属性插值 ===
                    // 使用重心坐标插值各种顶点属性（颜色、法向量、纹理坐标、世界坐标）
